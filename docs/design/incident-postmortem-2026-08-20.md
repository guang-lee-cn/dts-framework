...
| 措施 | 资产载体 | 状态 |
|---|---|---|
| 桶测试脚本加 `ulimit -v 3145728` 内存护栏 | **工具**（新路径首跑必带帽，不依赖记性） | ✅ 已落地（复盘审计当日闭环，ctest 带帽全过） |
...
## 6. 复盘审计（incident-review 纪律首跑，2026-08-20）

> 用 skills 仓库 review/incident-review.md 全流程重审本案：§1-5 为初版复盘，本节为纪律对照后的补充与闭环。

**Phase 1 响应期纪律缺位（补记）**：
- 缓解优先未执行：08-17 首崩后未隔离触发源（bucket 测试继续在 ctest 里裸奔），直接导致 08-20 两次重演——本次事故最大的响应期失误
- 首轮排查未跨 boot 搜硬故障史（第二轮 5 秒锁定），该教训已固化为 SOP

**5-Why 正式版（含停止判据检验）**：
```
1 为什么崩机？       收线程按 UINT32_MAX 预分配 4GB×3 通道，两进程吃满 21.5G
2 为什么按该值分配？  抄内置 SHM 的 max_recv_buffer_size 返回值，未核对消费契约
3 为什么未核对？     参考实现同值不炸（SHM 段内零拷贝读），"抄一半"——缺接口语义查证步骤
4 为什么测试没拦住？  唯一能触发该路径的测试此前全走 Skip 分支；沙箱/开发机环境分叉未识别
5 为什么首跑无护栏？  无"新路径首跑带帽"SOP，测试脚本无资源上限
→ 全部落到可修改机制（钳制/护栏/SOP/契约），无一层是"人会小心"——停止判据满足
```

**鱼骨图发散检视（人机料法环）**：人（抄参考未验契约）✓ / 机（swap=0 + VM 无上限放大器）✓ /
料（FastDDS min() 两端无界形同虚设 + UINT32_MAX 无文档警示）✓ / 法（Skip 分支验证假阳性 +
首跑无护栏）✓ / 环（环境漂移无盘点、记录误导）✓——**未发现新的未识别原因**；测试脚本对
skip/真跑分支的显式打印已提供可见性，不立新行动项。
