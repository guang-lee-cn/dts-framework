...
| 项 | 状态 | 验收标准 |
|---|---|---|
| RT 权限缺失 fail-fast | 待实现（当前降级 Warn） | 无 CAP_SYS_NICE 时拒绝启动或显式告警（产品决策） |
| 远程 OAM 通道（DDS 控制通道） | 待实现（D2/R5 未落地） | 网管可远程配置/状态采集 |
| 指标出口（丢包/jitter/超时 → OAM） | 部分（get_data_stats 本地） | 结构化指标可被网管拉取 |
| 打包（RPM/容器 + 版本锁定） | 待做 | 一键部署脚本 + FastDDS 3.6.2 锁定 |
| 看门狗/进程守护 | 待做 | dts 崩溃自动拉起 + 状态上报 |
| 内存核算 | 待做 | 目标硬件实测 RSS（当前 ~114MB 基线）+ 桶段 64MB 计入 |
