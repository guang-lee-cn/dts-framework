...
- **bootstrap Run 化**：`dts::Run(cfg)` 阻塞常驻 + `dts::Stop()`（cv 唤醒），`Process{comm, Worker×3}` 门面，Worker 自包含（ThreadCtx + detsched 句柄 + 订阅集合）
- **契约**：contracts/ 五份（detmw/infrastructure/contexts/bootstrap/detsched）
- **日志设计**：TsRotatingSink（时间戳文件名+5MB 切分+总量 5G 删旧），每线程分文件，共享单线程池
...
- [ ] **detmw 收尾**：`publish_internal` 当前仍走 transport，真正的 mailbox 直通（免序列化）待落地
